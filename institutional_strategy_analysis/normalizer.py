...
# Each entry: keyword (substring to search in sheet name) → canonical display name.
# Order matters — longer/more specific keywords should come first.
# Adding a manager here is the single source of truth for both loader and normalizer.

CANONICAL_MANAGERS: list[tuple[str, str]] = [
...
def normalize_manager_name(raw: str) -> tuple[str, str]:
    """
    Match raw sheet-name / cell text against CANONICAL_MANAGERS.
    Returns (canonical_name, match_type) where match_type is one of:
      "exact"      – keyword found verbatim in raw
      "fuzzy"      – found after stripping punctuation/spaces
      "unresolved" – no match; raw name kept as-is
    """
    s = _c(raw)
    # exact substring match
    for keyword, canonical in CANONICAL_MANAGERS:
        if keyword in s:
            return canonical, "exact"
    # fuzzy: strip punctuation and try again
    s_stripped = re.sub(r'[\s"\'״׳\-_]', "", s)
    for keyword, canonical in CANONICAL_MANAGERS:
        kw_stripped = re.sub(r'[\s"\'״׳\-_]', "", keyword)
        if kw_stripped and kw_stripped in s_stripped:
            return canonical, "fuzzy"
    # unresolved: keep the raw value so nothing is silently lost
    return s, "unresolved"
...
def _infer_meta(sheet_name: str) -> dict:
    s = _c(sheet_name)
    canonical, match_type = normalize_manager_name(s)
    track = "כללי"
    for pat, val in _TRACK_HINTS.items():
        if pat in s:
            track = val
            break
    return {"manager": canonical, "track": track, "manager_match": match_type}
    ...
    for _, row in raw.iterrows():
        ...
        for col in alloc_cols:
            ...
            rows.append({
                ...
                "track":            meta["track"],
                "manager_match":    meta.get("manager_match", "unknown"),
